...
| `openxr:monitors_follow_session` | string | `visible` | when XR monitors behave like **unplugged external monitors** (disabled — workspaces evacuate, then return by name like replugging a display). `off` = never (always-present); `session` = while no session exists; `visible` (default) = while the headset is **not being worn**. On runtimes with `XR_EXT_user_presence` (WiVRn) this is the real donned/doffed signal, so a **shelf session never plugs** even though WiVRn briefly drives it to FOCUSED at startup; otherwise it falls back to VISIBLE/FOCUSED + the settle guard below. Legacy `false/0`→off, `true/1`→session. |
| `openxr:monitor_unplug_grace_ms` | int | `20000` | with `monitors_follow_session = visible`, ms the headset must stay doffed/not-present before its monitors unplug (anti-flap for a quick doff-and-don). Donning re-plugs immediately. |
| `openxr:monitor_plug_settle_ms` | int | `1500` | `monitors_follow_session = visible` on a runtime **without** `XR_EXT_user_presence` only: ms the session must stay continuously VISIBLE before the first plug (suppresses the session-create visibility blip). Ignored when presence is available and after the first plug. `hyprctl openxr status` shows the `presence:` signal. |
