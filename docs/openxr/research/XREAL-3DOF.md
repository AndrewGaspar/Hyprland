...
| V2.5 | **DONE — motion-judder pass (2026-07-15).** `95b320d41` cherry-picked (gfx-path scanout compensation; the compute path we actually run already had it) **plus the two pieces that make compensation real here**: the `xreal_air` driver now publishes world-space angular velocity with every IMU sample (enables true forward pose prediction — the pre-fix pose was ~10-20ms stale at photon time, the main motion-judder source) and implements `get_compositor_info` (rolling-scanout TOP_TO_BOTTOM, 1080/1125 of the frame period per the native 3D EDID DTD: 3840x1080@60, vtotal 1125). Direct-mode pacing tuned via the `xr --direct` drop-in: `U_PACING_COMP_MIN_TIME_MS=8` (measured: 3.33ms default budget vs 6.5ms worst frame under iGPU contention = ~6% of frames slipping exactly one 60Hz FIFO period — 649 misses/3min loaded, ~0 idle) and `U_PACING_COMP_PRESENT_TO_DISPLAY_OFFSET_MS=1` (begin-of-scanout pose target ~= the FIRST_PIXEL_OUT vblank the fake pacer locks to; RADV has no VK_GOOGLE_display_timing so the fake pacer is the only option). A/B kill-switches: `XREAL_AIR_ANGULAR_VELOCITY=false`, `XREAL_AIR_SCANOUT_COMP=false` (env, e.g. in `~/.config/xreal/monado.env`). | **S** |
