...
| [28-spatial-model](28-spatial-model.md) | **the spatial model** — frames, hydration, honest failure: why every placement bug was a coordinate named against a frame that did not survive the event; STAGE as the missing room frame; bindings {frame, pose, provenance, hydration policy, don policy}; first-class seat; runtime/client split and `XR_EXT_stationary_reference_space` as the flagship extension | **tranche 1 built** 2026-08-27 (branch `spatial-model-t1`): W4 + H2 + S1 + M0 instrumentation + the §3.4 hygiene items, gtests 1091→1103, unattended — see §14 for the addendum, the simplification audit and what is next. M0's attended probe still pending; Tracks H3/H5/S2/W remain |
