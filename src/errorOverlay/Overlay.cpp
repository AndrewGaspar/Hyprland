...
void COverlay::createQueued() {
    ...
    *m_fadeOpacity = 1.f;
    ...
    const int         FONTSIZE = std::clamp(sc<int>(10.f * ((PMONITOR->paneSize().x * SCALE) / 1920.f)), 8, 40);
    ...
    const float barWidth     = std::max<float>(1.F, sc<float>(PMONITOR->paneSize().x) - m_outerPad * 2.F);
    ...
    m_damageBox = {
        ...
        sc<int>(PMONITOR->m_position.y + (TOPBAR ? 0 : PMONITOR->paneSize().y - (m_lastHeight + m_outerPad * 2.F))),
        sc<int>(PMONITOR->paneSize().x),
...
void COverlay::draw() {
    ...
    const float barWidth = std::max<float>(1.F, sc<float>(PMONITOR->paneSize().x) - m_outerPad * 2.F);
    const float barY     = TOPBAR ? m_outerPad : PMONITOR->paneSize().y - m_lastHeight - m_outerPad;
    const CBox  barBox   = {m_outerPad, barY, barWidth, m_lastHeight};

    m_damageBox.x      = sc<int>(PMONITOR->m_position.x);
    m_damageBox.width  = sc<int>(PMONITOR->paneSize().x);
    m_damageBox.height = sc<int>(m_lastHeight + m_outerPad * 2.F);
    m_damageBox.y      = sc<int>(PMONITOR->m_position.y + (TOPBAR ? 0 : PMONITOR->paneSize().y - m_damageBox.height));
