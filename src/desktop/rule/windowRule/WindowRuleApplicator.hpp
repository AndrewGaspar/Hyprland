...
#define DEFINE_PROP(type, name, def, eff)                                                                                                                                          \
  ...
  public:                                                                                                                                                                          \
    ...
    eWindowRuleEffect name##Effect() {                                                                                                                                             \
        ...
        // stereo content (research/24 §5.3, WP S1): a packed Render::Stereo::SDeclaration. Integer
        // rather than a struct because the renderer reads it per surface per frame and because the
        // rule engine's value variant is a fixed set of types.
        DEFINE_PROP(Config::INTEGER, stereo, 0, WINDOW_RULE_EFFECT_STEREO)
